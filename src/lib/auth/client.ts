...
export async function signIn(
  ...
  try {
    const { endGuestSession } = await import("@/lib/game/identity");
    endGuestSession();
  } catch {
    /* ignore */
  }
...
export async function signOut(redirectTo = "/"): Promise<void> {
  const { beginGuestSession } = await import("@/lib/game/identity");
  beginGuestSession();
  const dest = redirectTo.startsWith("/") ? redirectTo : "/";
  const guestDest = dest.includes("?") ? `${dest}&as=guest` : `${dest}?as=guest`;
  try {
    await runSignOut({
      livePreview: inLivePreview(),
      hasBearer: Boolean(getBearerToken()),
      ...
      clearToken: () => setBearerToken(null),
      redirect: () => {
        window.location.replace(guestDest);
      },
    });
  } catch {
    setBearerToken(null);
